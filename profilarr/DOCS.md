...
4. Add this repository URL: `https://github.com/rigerc/ha-apps`
5. Find "Profilarr" in the list
6. Click on the app and then click "Install"
...
After starting the app, access Profilarr through the Home Assistant interface:

1. **Via Ingress** (primary method): Click "Open Web UI" in the app info page or use the sidebar panel
...
2. **Direct access** (optional): If you have mapped port 6868, you can navigate to `http://homeassistant.local:6868`
   - Requires port mapping in app configuration
   - Use only if you need direct API access or prefer external access

### Initial Setup

1. Start the app and access the web UI
...
Profilarr stores its data in the app configuration directory:
...
**Note**: For external API access or automation from outside Home Assistant, you may want to map port 6868 in the app network configuration.
...
4. Ensure no port conflicts with other apps (if using port mapping)

### Cannot access Web UI

1. Verify the app is running (check status and logs)
2. **Primary method**: Use ingress by clicking "Open Web UI" in the app panel
3. Check app logs for nginx errors
...
- [GitHub Repository](https://github.com/rigerc/ha-apps)
- [Profilarr Upstream](https://github.com/Dictionarry-Hub/profilarr)

## Credits

This app wraps the excellent [Profilarr](https://github.com/Dictionarry-Hub/profilarr) project by santiagosayshey.
